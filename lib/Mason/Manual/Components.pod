...
'/foo/bar.m' refers to the file '/opt/web/comps/foo/bar.m'.
...
L<Mason::Interp/run>, which in turn may be called from a PSGI handler or an MVC
framework view depending on your setup. See L<Mason::Manual::RequestDispatch>
...
or via the L<Mason::Request/comp> or L<Mason::Request/scomp> methods:
...
=head2 Components are read-write by default
...
=head2 Declaring incoming attributes - C<< <%args> >>
...
The '$.' prefix on each line is optional and harmless. This is equivalent to
the following Moose syntax:
...
=head2 Declaring internal attributes - C<< <%shared> >>
...
C<< $.cmeta->args >> will return a hashref of all of the attributes passed to
the component when it was created/called.
