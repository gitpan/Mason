...
L<Mason::Request/apply_filter> can be used to manually apply a filter to a
string. It returns the filtered output.
...
Or if you want them available to all components, put them in C<Base.pm> at the
